	...
	var Game = function () {
	  ...
	  _createClass(Game, [{
	    ...
	    value: function createEnemies() {
	      var lvl = this.level;
	      for (var i = 0; i < lvl; i++) {
	        var r1 = Math.floor(Math.random() * 400) + 50;
	        var r2 = Math.floor(Math.random() * 400) + 50;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _inventory = __webpack_require__(102);
	...
	var Entity = function () {
	  ...
	  _createClass(Entity, [{
	    ...
	    value: function checkBounds(pos) {
	      ...
	      var xMax = 480 - bounds.width;
	      var yMax = 475 - bounds.height;
	      var min = this.enemy && this.body.scaleX === -1 ? 25 + bounds.width : 25;
	  ...
	  _createClass(Enemy, [{
	    ...
	    value: function step(pos) {
	      ...
	      if (dis < 100) {
	        ...
	        var dir = [-vec[0] / Math.abs(vec[0]), -vec[1] / Math.abs(vec[1])];
	    ...
	    value: function dropItem() {
	      var stats = {};
	      stats[_util_func.sample[STATS]] = this.lvl;
	      if (this.body.scaleX !== -1) {
	        stats.x = this.pos[0];
	      } else {
	        stats.x = this.pos[0] - 33;
	      }
	      stats.y = this.pos[1];
	      var item = new _item2.default(stats, this.newBag());

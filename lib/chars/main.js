...
export default class Character extends Entity{
  ...
  setItems(name){
    ...
    if(char && char.items){
      ...
      for (let i = 0; i < stats.length; i++) {
        this[stats[i]] *= (12*Math.log(buffs[stats[i]] + 9) -10)/10;
      }
  ...
  handleAddBuff(e){
    ...
    if(this.prestige.tokens >= price){
      ...
      $("#tokens").text(this.prestige.tokens);
      $("." + stat + "Buff .multiplier").text(Math.floor(12*Math.log(buffs.maxHp + 9) -10)/10 + "x");
      $("." + stat + "Buff .price").text("$ " + Math.floor(Math.exp(buffs.maxHp / 2) + 10));
    }
  }

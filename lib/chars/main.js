...
export default class Character extends Entity{
  ...
  setItems(name){
    ...
    $(".maxHpBuff .multiplier").text(buffs.maxHp * 1.5 + "x");
    $(".attBuff .multiplier").text(buffs.att * 1.5 + "x");
    $(".defBuff .multiplier").text(buffs.def * 1.5 + "x");
    $(".spdBuff .multiplier").text(buffs.spd * 1.5 + "x");
    $(".maxHpBuff .price").text("$ " + buffs.maxHp * 10);
    $(".attBuff .price").text("$ " + buffs.att * 10);
    $(".defBuff .price").text("$ " + buffs.def * 10);
    $(".spdBuff .price").text("$ " + buffs.spd * 10);
  }
  ...
  getEquipped(){
    ...
    const char = { lvl: this.lvl, prestige: this.prestige, items: items };
  ...
  handleAddBuff(e){
    ...
    const price = parseInt($(e.target).prev().text());
    if(this.prestige.tokens >= price){
      this.prestige.tokens -= price;
      this.prestige.buffs[stat] *= 1.5;
      $("#tokens").text(this.prestige.tokens);
      $("." + stat + "Buff .multiplier").text(this.prestige.buffs[stat] * 1.5);
      $("." + stat + "Buff .price").text(this.prestige.buffs[stat] * 10);
    }
  }

...
export default class Game{
  ...
  blowUp(r){
    ...
    objEach(enemies, (enemy) => {
      if(distance(enemy.pos, player.pos) < 60 + (r * 5)){
        const body = enemy.takeDmg(1000, [0,0]);
        if(body){
          dead.push(enemy);
  ...
  setName(e){
    ...
    const pin = $("#pin-input")[0].value;
    if(name === "" || pin === ""){
    }else{
      this.player.name = name + "_-_" + pin;
      ...
      const game = this;
      db.ref(`users/${this.player.name}`).once('value').then((obj) => {
